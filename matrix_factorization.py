...
class MatrixFactorization(Regressor):
    def __init__(self, config):
        ...
        self.Q = None #latent matrix for users
        self.P = None #latent matrix for items
    ...
    def calc_regularization(self):
        """
        calc  γ · ( Σ u b u 2 + Σ i b i 2 + \\Q\\2 + \\P\\ 2 )
        """
        return self.gamma * ((self.user_biases ** 2).sum() + (self.item_biases ** 2).sum() + (self.Q ** 2).sum() + (self.P **2).sum())
    ...
    def fit(self, X: np.array):
        ...
        self.Q = np.random.normal(0, 0.1, (self.n_users, self.k))
        self.P = np.random.normal(0, 0.1, (self.n_items, self.k))
    ...
    def run_epoch(self, X: np.array):
        ...
        for row in X:
            ...
            self.Q[user] += self.lr * (err * self.P[item] - self.gamma * self.Q[user])
            self.P[item] += self.lr * (err * self.Q[user] - self.gamma * self.P[item])
    ...
    def predict_on_pair(self, user: int, item: int):
        ...
        return self.global_bias + self.user_biases[user] + self.item_biases[item] + np.dot(self.Q[user], self.P[item])
